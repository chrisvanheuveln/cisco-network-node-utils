  ...
  def test_snmpcommunity_collection_empty
    ...
    if platform != :ios_xr
      ...
      config('no snmp-server community com1',
             ...
             'no snmp-server group network-operator v3 noauth',
             'no snmp-server community-map cisco target-list network-operator',
             'no snmp-server community-map cisco128lab target-list network-operator', # rubocop:disable Metrics/LineLength
             'no snmp-server community ciscogroupsetcom',
             'no snmp-server group network-operator v3 noauth',
             'no snmp-server community-map ciscogetgrp target-list network-operator', # rubocop:disable Metrics/LineLength
             'no snmp-server community-map ciscogroupsetcom target-list network-operator', # rubocop:disable Metrics/LineLength
             'no snmp-server community ciscoaclget IPv4 ciscoacl',
             'no snmp-server group vdc-admin v3 noauth',
             'no snmp-server group network-admin v3 noauth',
             'no snmp-server group network-operator v3 noauth')
    end
